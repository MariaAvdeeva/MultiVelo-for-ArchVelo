...
def regress_func(c, u, s, m, mi, im, gpdist, embed, conn, v, pl, sp, pdir, fa,
    ...
    try:
        ...
        if v >= 1:
            print(f"Skipping gene {gene} due to numerical error: {e}")
        
        # 3. Return the "failed" tuple (This is identical to the library's built-in low-quality return)
        return (np.inf, np.nan, '', (np.zeros(3), np.zeros(4), 0, 0, 0, 0),
                np.zeros(3), np.zeros(len(u)), np.zeros(len(u)),
                np.zeros((len(u), 3)), (-1.0, 0, 0, 0),
                (np.zeros((1, 3)), np.zeros((1, 3)), np.zeros((1, 3)), 0, 0, 0, 0))
